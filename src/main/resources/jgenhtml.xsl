...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="cssName" select="'html/jgenhtml.css'"/>
	<xsl:param name="jsName" select="'html/jgenhtml.js'"/>
	...
	<xsl:template name="buildHeader">
		...
		<h1>Verilator - code coverage report</h1>
				<div class="header">
					<div class="row">
						<dl class="pageView">
							<dt>Current view</dt>
							<dd>
								<xsl:choose>
									<xsl:when test="@package">
										<a>
											<xsl:attribute name="href">
												<xsl:value-of select="concat($base, 'VerilatorCoverage', $ext)"/>
											</xsl:attribute>
									...
									<xsl:when test="self::descriptions">
										<a>
											<xsl:attribute name="href">
												<xsl:value-of select="concat('VerilatorCoverage', $ext)"/>
											</xsl:attribute>
											<xsl:text>top level</xsl:text>
										</a>
	...
	<xsl:template match="sources">
		...
		<table class="VerilatorCoverage">
			<xsl:attribute name="class">
				<xsl:text>VerilatorCoverage</xsl:text>
					...
					<xsl:when test="@pathname">
						<a>
							<xsl:attribute name="href">
								...
								<xsl:value-of select="concat(@pathname, '/VerilatorCoverage', $ext)"/>
							</xsl:attribute>
